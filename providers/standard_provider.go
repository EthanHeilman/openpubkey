...
// keyBindingOptions returns the zitadel/oidc RP options needed to run the
// OpenID Connect Key Binding protocol for this token request. When no key
// binding signer is configured (the common StandardOp case) it returns nil,
// leaving the request unchanged. Otherwise rp.WithKeyBinding wires up the
// bound_key scope, the dpop_jkt authorization parameter, the per-request DPoP
// proof, and verification that the returned ID Token is bound to the key.
